...
export function usePlateImage(emirate: string, code: string, number: string) {
...
export function usePlateGenerator() {
  ...
  const downloadPlate = useCallback(async () => {
    ...
    try {
      ...
      if (current4kRef.current) {
        const filename = `UAE_Plate_${emirate}_${plateCode}_${plateNumber}.png`;
  ...
  }, [emirate, plateCode, plateNumber, renderPlate]);
